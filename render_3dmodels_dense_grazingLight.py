...
def render_core(args: Options, groups_id = 0):
    ...
    def gen_grazing_point_lights(
        seed: Optional[int],
        N: int,
        min_dist_to_origin: float,
        max_dist_to_origin: float,
        min_elevation_deg: float = 4.0,
        max_elevation_deg: float = 18.0,
    ):
        """Sample point lights from grazing angles around the object.

        z-up convention:
          - azimuth is uniform in [0, 2pi)
          - elevation is low positive angle above horizon (side lighting)
        """
        if N <= 0:
            return []
        ...
        pts = []
        for _ in range(N):
            r = rng.uniform(min_dist_to_origin, max_dist_to_origin)
            az = rng.uniform(0.0, 2.0 * math.pi)
            elev = math.radians(rng.uniform(min_elevation_deg, max_elevation_deg))
            xy = r * math.cos(elev)
            x = xy * math.cos(az)
            y = xy * math.sin(az)
            z = r * math.sin(elev)
    ...
    white_pls = gen_grazing_point_lights(
        seed=seed_white_pl,
    ...
    rgb_pls = gen_grazing_point_lights(
        seed=seed_rgb_pl,
        N=args.num_rgb_pls,
        min_dist_to_origin=4.0,
        max_dist_to_origin=5.0,
        min_elevation_deg=4.0,
        max_elevation_deg=16.0,
    )
    ...
    multi_pls = gen_grazing_point_lights(
        seed=seed_multi_pl,
    ...
    combined_pls = gen_grazing_point_lights(
        seed=seed_combined,
        N=num_point_lights,
        min_dist_to_origin=3.5,
        max_dist_to_origin=5.0,
