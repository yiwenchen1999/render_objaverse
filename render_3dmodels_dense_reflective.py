...
def render_core(args: Options, groups_id = 0):
    ...
    def make_reflective_material(style: str, material_name="Reflective_Material"):
        """Create one reflective material preset on Principled BSDF.

        style:
            - "specular": very strong dielectric highlights
            - "glossy": glossy paint-like surface (non-metal)
            - "metallic": brushed-like metal
        """
        ...
        if style == "specular":
            # High-specular dielectric (strong highlights, low roughness).
            ...
            # Glossy non-metal look (slightly softer highlight than specular mode).
            bsdf.inputs["Base Color"].default_value = (0.45, 0.47, 0.52, 1.0)
            bsdf.inputs["Metallic"].default_value = 0.0
            ...
            if "Anisotropic Rotation" in bsdf.inputs:
                bsdf.inputs["Anisotropic Rotation"].default_value = 0.35
            if "Coat Weight" in bsdf.inputs:
    ...
    def override_mesh_materials(material) -> None:
        """Assign one material to every mesh object in scene."""
    ...
    material_rng = random.Random(None if args.seed is None else args.seed + groups_id * 10007)
    material_style = material_rng.choice(["specular", "glossy", "metallic"])
    print(f"[material] using reflective style: {material_style}")
    reflective_material = make_reflective_material(
        style=material_style,
        material_name=f"Reflective_{material_style}",
    )
    override_mesh_materials(reflective_material)
    ...
    if args.num_combined_lights > 0:
        ...
        point_powers = [random.uniform(300, 1500) for _ in range(num_point_lights)]
        point_colors = [[1.0, 1.0, 1.0] for _ in range(num_point_lights)]
        
        area_light_pos = area_light_positions[0]
        area_light_power = random.uniform(300, 1500)
        area_light_size = random.uniform(5., 10.)
